...
## 8. Suite de tests de non-régression

`poc/widget.html#test` (ou `python -m http.server 8010` puis `http://localhost:8010/poc/widget.html#test`) exécute une suite interne d'environ 238 tests, hors ligne, sans document Grist : l'API `grist.docApi` et `fetch()` sont simulées à l'intérieur même du widget, jamais de vraie requête réseau ni de vraie modification de document. Couvre les fonctions pures (analyse de lien, décodage des options, conversion des couleurs), le rendu de chaque type de question, la bascule des blocs de la carte d'édition selon le type choisi, l'enregistrement d'une question de chaque type, les conditions d'affichage combinées en ET/OU, l'import des champs natifs, la réinitialisation, la création d'un formulaire vide (table existante et nouvelle table), la détection automatique de clé de partage, la liste déroulante de formulaires, et la construction des champs envoyés à l'API pour une soumission mêlant tous les types de question (pièces jointes comprises).

À relancer après toute modification de `widget.html` : la page affiche le nombre de tests réussis en tête, et le détail de chaque échec (attendu / obtenu) dans le journal en dessous. Un test qui échoue à cause d'un état de navigateur laissé par un essai manuel précédent (thème sombre resté actif, `localStorage` non vidé) n'est pas un vrai échec : recharger la page dans un onglet neuf avant de conclure à une régression.
