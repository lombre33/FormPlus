...
`poc/widget.html#test` (ou `python -m http.server 8010` puis `http://localhost:8010/poc/widget.html#test`) exécute une suite interne d'environ 180 tests, hors ligne, sans document Grist : l'API `grist.docApi` et `fetch()` sont simulées à l'intérieur même du widget, jamais de vraie requête réseau ni de vraie modification de document. Couvre les fonctions pures (analyse de lien, décodage des options, conversion des couleurs), le rendu de chaque type de question, la bascule des blocs de la carte d'édition selon le type choisi, l'enregistrement d'une question de chaque type, l'import des champs natifs, la réinitialisation, la création d'un formulaire vide (table existante et nouvelle table), la détection automatique de clé de partage, la liste déroulante de formulaires, et la construction des champs envoyés à l'API pour une soumission mêlant tous les types de question.
