...
Deux façons de limiter ce chrome pour un widget personnalisé, aucune ne l'élimine totalement :
- Réduire au minimum, sans aucun champ, et positionner hors du premier écran la section Formulaire natif qui porte la clé ; elle n'a besoin que d'exister, pas d'être vue. Il reste alors le mince bandeau de page et l'en-tête propre à la section du widget personnalisé.
- À moyen terme, demander à l'administrateur de l'instance (DINUM) une règle CSS globale ciblant la classe `interface-singlePage` posée sur `<body>` quand `style=singlePage` est actif, via la variable d'environnement serveur `APP_STATIC_INCLUDE_CUSTOM_CSS` (self-hosted). Hors du contrôle du widget, à proposer séparément.
